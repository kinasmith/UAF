/*****************************
   On Turn-On:
   Preheats sensor for 10 minutes
   Then sends Temps every 30 sec
...
uint16_t heatTime = 6; //in seconds
uint8_t ledState = 0;
...
void setup() {
  ...
  digitalWrite(HEAT_EN, HIGH); //turn on heater
...
void loop() {
  ...
  digitalWrite(LED, HIGH); //Turns ON led to signal that Transmission has started
  ...
  digitalWrite(LED, LOW); //turn LED off to signal completion of transmission
